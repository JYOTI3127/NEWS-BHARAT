...
class MetaEngine:
    ...
    def for_article(article) -> dict:
        ...
        news_keywords = ", ".join(dict.fromkeys(filter(None, [article.focus_keyword, *tags_list])))
        secondary_keywords = ", ".join(
            k.strip() for k in (article.secondary_keywords or "").split(",") if k.strip()
        )
        ...
        first_cat = article.categories.first()
        cat_name  = str(first_cat) if first_cat else ""
        modified_at = getattr(article, "updated_at", None) or article.published_at or article.created_at

        author_name = (
            (article.author_display_name or "").strip()
            or article.author.get_full_name()
            or article.author.username
        )
        ...
        return {
            ...
            "focus_keyword": (article.focus_keyword or "").strip(),
            ...
            "article_meta": {
                "published_time": _iso(article.published_at),
                "modified_time":  _iso(modified_at),
                "author":         author_name,
                "section":        cat_name,
                "tags":           tags_list,
            },
    ...
    def render_head(meta: dict, schemas: list = None) -> str:
        ...
        article_meta = meta.get("article_meta") or {}
        article_tags = article_meta.get("tags") or []
        article_meta_tags = "\n".join(
            filter(
                None,
                [
                    f'  <meta property="article:published_time" content="{e(str(article_meta.get("published_time")))}">'
                    if article_meta.get("published_time") else "",
                    f'  <meta property="article:modified_time" content="{e(str(article_meta.get("modified_time")))}">'
                    if article_meta.get("modified_time") else "",
                    f'  <meta property="article:author" content="{e(str(article_meta.get("author")))}">'
                    if article_meta.get("author") else "",
                    f'  <meta property="article:section" content="{e(str(article_meta.get("section")))}">'
                    if article_meta.get("section") else "",
                    *[
                        f'  <meta property="article:tag" content="{e(str(tag))}">'
                        for tag in article_tags if tag
                    ],
                ],
            )
        )
  ...
  {"<meta name='news_keywords' content='" + e(meta['news_keywords']) + "'>" if meta.get('news_keywords') else ""}
  {"<meta name='focus_keyword' content='" + e(meta['focus_keyword']) + "'>" if meta.get('focus_keyword') else ""}
  {"<meta name='secondary_keywords' content='" + e(meta['secondary_keywords']) + "'>" if meta.get('secondary_keywords') else ""}
